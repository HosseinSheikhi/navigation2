  ...
  const nav2_costmap_2d::Costmap2D * costmap)
{
  ...
  float dist_heuristic = 0.0;
  if(motion_model_!=MotionModel::OMNI)
    dist_heuristic = getDistanceHeuristic(node_coords, goal_coords, obstacle_heuristic);
  else{
    const unsigned int size_x = costmap->getSizeInCellsX();
    const unsigned int goal_index = goal_coords.y  * size_x + goal_coords.x;
    dist_heuristic = distanceHeuristic2D(goal_index, size_x, node_coords.x, node_coords.y);
  }
