...
{
  RCLCPP_INFO(logger_, "CeilingLayer: initialized");
  ...
  ceiling_ROI_srv_ = node->create_service<custom_roi_srv::srv::ROI>("get_ceiling_roi",
                                                                     std::bind(&CeilingLayer::handle_ceiling_roi_service,
                                                                               this,
                                                                               std::placeholders::_1,
                                                                               std::placeholders::_2),
...
void nav2_ceiling_perception_plugin::CeilingLayer::handle_ceiling_roi_service(std::shared_ptr<custom_roi_srv::srv::ROI::Request> request,
  ...
  RCLCPP_INFO(logger_, "CeilingLayer: ceiling roi service called");
...
{
  RCLCPP_DEBUG(logger_, "CeilingLayer::onFootprintChanged(): num footprint points: %lu",
    ...
    int max_j)
{
  std::lock_guard<Costmap2D::mutex_t> guard(*getMutex());
//  #if DEBUG
//  RCLCPP_INFO(logger_, "CeilingLayer: Update cost called");
//  #endif
  ...
  if(!(worldToMap(roi_min_x_, roi_min_y_,map_min_x, map_min_y) && worldToMap(roi_max_x_, roi_max_y_,map_max_x,map_max_y))){
    RCLCPP_WARN(logger_, "CeilingLayer: costmap layered is not resized yet");
    return;
  }
...
//  #if DEBUG
//  RCLCPP_INFO(logger_, "CeilingLayer: update with new one");
//  #endif
  for (unsigned int i = 0; i < ceiling_size_y_; ++i)
...
//  #if DEBUG
//  RCLCPP_INFO(logger_, "CeilingLayer: Update cost end");
//  #endif
}
